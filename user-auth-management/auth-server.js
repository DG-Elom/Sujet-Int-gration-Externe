...
app.patch('/update', verifiedConnection, async (req, res) => {
    ...
    if(!userId) {
        ...
        return res.status(400).json({ statut: "Erreur", mesage: "L'identifiant de l'utilisateur n'a pas été trouvé"})
    }
    ...
    if(!req.body || (!username && !newPassword)) {
        ...
        return res.status(400).json({ statut: "Erreur", mesage: "Le nom et le mot de passe de l'utilisateur n'ont pas été trouvés"})
    }
