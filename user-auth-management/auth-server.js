    ...
    try {
        ...
        await req.db.execute(
            ...
            [
                null,
                username,
                // On ne stocke password dans la base de données, parce qu'on ne stocke pas les mots de passe en clair
                // On stocke hash
                hashedpassword
            ]
        );
...
app.post('/login', validateRegisterAndLogin, async (req, res) => {
    ...
    try {
        // Je hache le mot de passe à l'aide de "bcrypt". Le "10" signifie le nombre de fois que l'algorithme de hash est effectué. Plus le nombre est grand plus la sécurité est grande mais plus le temps d'exécution est long.
        // C'est pourquoi on utilise await afin d'attendre la fin d'exécution de bcrypt qui peut être plus ou moins longue selon le nombre de fois que l'algorithme de hash est effectué.
        const hashedpassword = await bcrypt.hash(password, 10);

        // On vérifie si l'utilisateur existe et si le mot de passe correspond à l'utilisateur
        if(hashedpassword != "jgk") {
            // Voici ce que j'envoie si l'utilisateur n'existe pas ou s'il n'y a pas de correspondance entre les deux mots de passe. Statut 500 pour spécifié que le serveur n'est pas disponible.
